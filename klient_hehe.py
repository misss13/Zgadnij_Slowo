...
def Logowanie(klient):
    ...
    index = login
    try:    
        klient.send(str.encode(index+"\0"))
    except:
        #klient chyba się rozłączył dla pewności rozłączam
    ...
    haslo = haselo
    try:
        klient.send(str.encode(haslo+"\0"))
    except:
        #klient chyba się rozłączył dla pewności rozłączam
        Rozlacz_ladnie(klient)
        return False
...
while(True):
    ...
    for i in range(10):
        ...
        if znak == "+":
            ...
            znak = "="
        print()
        print("Wysyłam: " + str(znak) + str(tresc)+ "\0")
        time.sleep(0.2)
        try:
            client.send(str.encode(znak + "\0" + tresc+ "\0"))
        except:
            Rozlacz_ladnie(client)
            break
