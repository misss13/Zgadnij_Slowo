...
def Czy_kolejna_runda(id_gry):
    """Czeka i sprawdza czy można już zacząć kolejną runde tak - True wyjście z pętli, nie - czekam"""
    global Slownik_synchronizacji

    while Slownik_synchronizacji[id_gry] == False:
        time.sleep(0.1)
    return True
...
def Obsluga_klienta(client, adres):
    ...
    if (czy_uwierzytelniony == False):
        ...
        for runda in range(ILOSC_RUND):
            #funkcja do upośledzonej synchroinizacji
            Czy_kolejna_runda(id_gry)
            #slowo zgadniete
...
def Czasomierz():
    ...
    while True:
        ...
        if (Ilosc_graczy >= MAX_UZYTKOWNIKOW) or (i>=trwanie_do_rundy):
            ...
            random.shuffle(Bierzaca_gra_gracze)
            start_new_thread(Gra,(Bierzaca_gra_gracze, liczba_graczy))
            #gra_wontek = threading.Thread(target=Gra, args=(Bierzaca_gra_gracze, liczba_graczy), daemon=True)
            #gra_wontek.start()
        time.sleep(2)
        i += 1
...
def Daj_znak_klientom(id_gry):
    """Czeka sobie 1.5 po każdej rundzie Slownik_synchronizacji{id_gry:false} na 0.5 ustawia true na Slownik_synchronizacji{id_gry:true}"""
    global Slownik_synchronizacji
    global ILOSC_RUND

    time.sleep(0.5)
    Slownik_synchronizacji[id_gry] = True
    for i in range(ILOSC_RUND):
        time.sleep(0.5)
        Slownik_synchronizacji[id_gry] = False
        time.sleep(2)
        Slownik_synchronizacji[id_gry] = True
...
def Gra(Bierzaca_gra_gracze, Ilosc_w_grze):
    ...
    start_new_thread(Daj_znak_klientom, (id_gry,))
    ...
    del Slownik_synchronizacji[id_gry]
